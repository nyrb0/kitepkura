...
const PageCompetiton = () => {
    ...
    const [expandedYear, setExpandedYear] = useState<string | null>(() => {
        const activeMenu = menuItems.find(item => item.subItems.some(sub => sub.href === currentPath));
    ...
    return (
        <div className='flex min-h-[100dvh] w-full items-center justify-center bg-background p-4 sm:p-6 lg:p-8'>
            ...
            <nav className='w-full max-w-2xl rounded-3xl border border-border bg-surface p-6 shadow-elevated sm:p-8 lg:p-10'>
                <h2 className='mb-6 text-center text-sm font-bold uppercase tracking-[0.3em] text-text-muted sm:text-base'>Жабык конкурстар</h2>

                <div className='flex flex-col gap-4'>
                    {menuItems.map(item => {
                        const isExpanded = expandedYear === item.id;
                        // Проверяем, активен ли какой-то из подразделов внутри этого года
                        const hasActiveChild = item.subItems.some(sub => sub.href === currentPath);
                            ...
                            <div key={item.id} className='flex flex-col'>
                                ...
                                >
                                    ...
                                    <svg
                                        ...
                                        className={`h-5 w-5 transition-transform duration-300 ${
                                            isExpanded ? 'rotate-180 text-brand' : 'text-text-muted group-hover:text-text'
                                        }`}
                                    >
                                ...
                                <div
                                    className={`grid transition-all duration-300 ease-in-out ${
                                        isExpanded ? 'grid-rows-[1fr] opacity-100 my-2' : 'grid-rows-[0fr] opacity-0 pointer-events-none'
                                    }`}
                                >
